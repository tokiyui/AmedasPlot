...
u300 = np.flip(grbs.select(parameterName='u-component of wind', level=500, forecastTime=6)[0].data()[0], axis=0)
v300 = np.flip(grbs.select(parameterName='v-component of wind', level=500, forecastTime=6)[0].data()[0], axis=0)
...
ttd = tmp700 * units('K') - mpcalc.dewpoint_from_relative_humidity((tmp700+273.15) * units('K'), rh700 / 100)
...
plt.fill_between(grid_lon_p, grid_lat_p, where=(ttd <= 3), color='lightgreen', alpha=0.3)
plt.fill_between(grid_lon_p, grid_lat_p, where=(ttd >= 15), color='yellow', alpha=0.3)
